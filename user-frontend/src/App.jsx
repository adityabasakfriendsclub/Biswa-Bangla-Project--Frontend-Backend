...
import React, { useState, useEffect } from "react";
import WelcomeSplash from "./pages/WelcomeSplash";
import SocialMissionPage from "./pages/SocialMissionPage"; // ✅ NEW
...
export default function App() {
  ...
  // 🔥 Welcome Splash State
  const [showSplash, setShowSplash] = useState(true);
  const [showSocialMission, setShowSocialMission] = useState(false); // ✅ NEW
  const [isInitializing, setIsInitializing] = useState(true);
  ...
  // ✅ Updated: Handle splash close - show Social Mission page
  const handleSplashClose = () => {
    setShowSplash(false);
    setShowSocialMission(true); // Show Social Mission page after splash
  };

  // ✅ NEW: Handle Social Mission close - navigate to login
  const handleSocialMissionClose = () => {
    setShowSocialMission(false);
    if (!user) {
      setCurrentPage("login");
    }
  };
  ...
  // ✅ SOCIAL MISSION PAGE (Shows after WelcomeSplash closes)
  if (showSocialMission) {
    return <SocialMissionPage onClose={handleSocialMissionClose} />;
  }
