...
// src/components/Copyright.jsx - UPDATED WITH FOOTER LINKS
export default function Copyright({
  organizationName = "Biswa Bangla Social Networking Services Club",
  showIcons = true,
  bgColor = "bg-gray-950",
  textColor = "text-gray-400",
  borderColor = "border-gray-800",
  onNavigate, // ✅ NEW: Navigation handler
}) {
  const currentYear = new Date().getFullYear();
  ...
  // ✅ Footer Legal Links
  const legalLinks = [
    { name: "Privacy Policy & Refund Policy", page: "privacy-policy" },
    {
      name: "Terms & Conditions And Refund Policy",
      page: "terms-and-conditions",
    },
    { name: "Community Guidelines", page: "community-guidelines" },
    { name: "Children and Minors Prohibited", page: "children-and-minors" },
  ...
  return (
    <footer
      className={`relative z-20 w-full ${bgColor} border-t ${borderColor}`}
    >
      {/* Government & Compliance Approvals Section */}
      <div className="py-8 sm:py-10 px-4 bg-gradient-to-r from-gray-950/50 via-gray-900/30 to-gray-950/50">
        <div className="max-w-6xl mx-auto">
          <h3 className="text-center text-white text-sm sm:text-base md:text-lg font-bold mb-6 tracking-wide">
            ✓ GOVERNMENT & REGULATORY APPROVALS
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-8">
            {[
              { icon: "🏛️", title: "Central Government", status: "Approved" },
              { icon: "🏢", title: "State Government", status: "Approved" },
              { icon: "💰", title: "GST", status: "Approved" },
              { icon: "🏭", title: "MSME", status: "Approved" },
            ].map((item, index) => (
              <div
                key={index}
                className="flex flex-col items-center justify-center p-4 sm:p-6 bg-gray-900/50 border border-gray-700/50 rounded-lg hover:border-cyan-500/50 transition duration-300 group"
              >
                <div className="text-2xl sm:text-3xl mb-2 group-hover:scale-110 transition duration-300">
                  {item.icon}
                </div>
                ...
                <p
                  className={`${textColor} text-xs mt-1 text-center font-light`}
                >
                  {item.status}
                </p>
              </div>
      ...
      {/* ✅ NEW: FOOTER LEGAL LINKS SECTION */}
      <div className="py-8 sm:py-10 px-4 bg-gray-900/30">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-4 mb-6">
            {legalLinks.map((link, index) => (
              <button
                ...
                className={`${textColor} hover:text-cyan-400 text-sm text-center py-2 px-4 rounded-lg hover:bg-gray-800/50 transition-all duration-300 cursor-pointer`}
              >
                {link.name}
              </button>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:hidden mb-6">
            {legalLinks.map((link, index) => (
              <button
                key={index}
                onClick={() => onNavigate?.(link.page)}
                className={`${textColor} hover:text-cyan-400 text-xs sm:text-sm text-center py-2 px-3 rounded-lg hover:bg-gray-800/50 transition-all duration-300 border border-gray-700/50 cursor-pointer`}
              >
                {link.name}
              </button>
            ))}
