...
//   const API_URL = "https://biswabanglasocialnetworkingservices.com/api";
...
// new
// ✅ FIXED VERSION - Properly handles host vs user redirect
// import React, { useState, useEffect } from "react";
// import { useNavigate, useLocation, Link } from "react-router-dom";
// import { ArrowLeft } from "lucide-react";
// import { useAuth } from "../../context/AuthContext";
...
//         if (type === "signup" && data.token && data.user) {
//           // ✅ CRITICAL FIX: Save token and user data
//           login(data.token, data.user);

//           console.log("✅ User verified and logged in:", data.user);
//           console.log("✅ Is Host:", data.user.isHost);
//           console.log("✅ Is Host Premium:", data.user.isHostPremium);

//           // ✅ CRITICAL FIX: Redirect based on user type
//           const redirectPath = data.user.isHost ? "/host/home" : "/dashboard";
...
// // FILE: frontend/src/components/auth/VerifyOtp.jsx
...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(`📡 Verifying OTP at: ${API_ENDPOINT}${endpoint}`);
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await fetch(`${API_ENDPOINT}/host/resend-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone }),
      });
